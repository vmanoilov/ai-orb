...
const TONE_MAPPING: Record<VisualTone, ToneParameters> = {
  Neutral: {
  ...
  Contemplative: {
    emissive: new THREE.Color(0x0a104a),
    metalness: 0.6,
    roughness: 0.2,
    bloom: 1.2,
    speedFactor: 0.5,
  ...
  Annoyed: {
    emissive: new THREE.Color(0x5e140a),
    metalness: 0.4,
    roughness: 0.4,
    bloom: 2.0,
    speedFactor: 1.8,
  ...
  Curious: {
    emissive: new THREE.Color(0x0a4a3b),
    metalness: 0.7,
    roughness: 0.1,
    bloom: 1.7,
    speedFactor: 1.2,
  ...
  Witty: {
    emissive: new THREE.Color(0x4a4a0a),
    metalness: 0.3,
    roughness: 0.05,
    bloom: 2.5,
    speedFactor: 1.5,
...
export class GdmLiveAudioVisuals3D extends LitElement {
  ...
  @property() visualTone: VisualTone = 'Neutral';
  ...
  private animation() {
    ...
    // Update visual tone
    const targetTone = TONE_MAPPING[this.visualTone] || TONE_MAPPING.Neutral;
    const lerpFactor = 0.05; // Speed of transition

    sphereMaterial.emissive.lerp(targetTone.emissive, lerpFactor);
    sphereMaterial.metalness +=
      (targetTone.metalness - sphereMaterial.metalness) * lerpFactor;
    sphereMaterial.roughness +=
      (targetTone.roughness - sphereMaterial.roughness) * lerpFactor;
    this.bloomPass.strength +=
      (targetTone.bloom - this.bloomPass.strength) * lerpFactor;
    ...
    if (sphereMaterial.userData.shader) {
      ...
      const f = 0.001 * targetTone.speedFactor;
  ...
  protected firstUpdated() {
    // FIX: Use this.renderRoot as shadowRoot is not available on this component's type.
